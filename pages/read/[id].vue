  ...
  <div v-else class="max-w-3xl mx-auto flex flex-col gap-6 pb-28">
    ...
    <div class="flex items-center gap-3 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-900">
      ...
      <UButton
        v-if="!generating"
        color="primary"
        :icon="allGenerated ? 'i-lucide-refresh-cw' : 'i-lucide-play'"
        :disabled="!selectedVoice"
        @click="handleGenerate"
      >
        {{ allGenerated ? 'Regenerate' : 'Generate Audio' }}
      </UButton>
      <template v-else>
...
const allGenerated = computed(() =>
  !!readData.value?.segments.length && readData.value.segments.every(s => s.audio_generated),
)

const exporting = ref(false)
const initialLoadDone = ref(false)
const autoGeneratedForRead = ref<number | null>(null)

// Set segments for audio player + auto-start generation on first load
watch(readData, async (data) => {
  ...
  // Auto-start generation if arriving with a voice param and no audio yet
  if (autoGeneratedForRead.value === data.id || generating.value) return
  ...
  autoGeneratedForRead.value = data.id
...
async function handleGenerate() {
  ...
  await generate(selectedVoice.value, undefined, allGenerated.value)
}
